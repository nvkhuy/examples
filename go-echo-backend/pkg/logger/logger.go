...
func (logger *Logger) WithRequestInfo(info *RequestInfo) *Logger {
	...
	if info.UserID != "" {
		fields = append(fields, zap.String(UserIDKey, info.UserID))
	}

	if info.UserRole != "" {
		fields = append(fields, zap.String(UserRoleKey, info.UserRole))
	}

	if info.GhostID != "" {
		fields = append(fields, zap.String(GhostIDKey, info.GhostID))
	}
...
func (logger *Logger) ErrorAny(data interface{}, msg ...string) {
	...
	case error:
		...
		logger.WithOptions(zap.AddCallerSkip(1)).Error(msg, zap.Any(StackKey, stack))
	default:
