...
func PaginatePost(c echo.Context) error {
	...
	params.Statuses = []enums.PostStatus{enums.PostStatusPublished}
	params.Language = cc.GetLanguage()
...
// @Security ApiKeyAuth
// @Failure 404 {object} errs.Error
// @Router /api/v1/posts/stats [get]
func GetPostStats(c echo.Context) error {
	var cc = c.(*models.CustomContext)
	var params repo.GetPostStatsParams
	...
	post, err := repo.NewPostRepo(cc.App.DB).GetPostStats(params)
	if err != nil {
		return eris.Wrap(err, err.Error())
	}
