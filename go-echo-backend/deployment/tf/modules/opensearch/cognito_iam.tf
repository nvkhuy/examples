...
resource "aws_iam_role" "cognito_es_role" {
  count              = var.cognito_enabled ? 1 : 0
  name               = "${var.name}_${var.env}_AmazonOpenSearchServiceCognitoAccess"
...
resource "aws_iam_role" "authenticated" {
  count = var.cognito_enabled ? 1 : 0
  name  = format("cognito_authenticated-%s-%s", var.name, var.env)
...
resource "aws_iam_role_policy" "authenticated" {
  count = var.cognito_enabled ? 1 : 0
  name  = format("authenticated_policy-%s-%s", var.name, var.env)
...
resource "aws_iam_role" "unauthenticated" {
  count = var.cognito_enabled ? 1 : 0
  name  = format("cognito_unauthenticated-%s-%s", var.name, var.env)
...
resource "aws_iam_role_policy" "unauthenticated" {
  count = var.cognito_enabled ? 1 : 0
  name  = format("unauthenticated_policy-%s-%s", var.name, var.env)
